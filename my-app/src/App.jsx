...
//   escalate:    (id, note) => fetch(`${API_BASE}/alerts/${id}/escalate`, {
...
//   <span style={{ padding:"2px 9px", borderRadius:2, fontSize:10, fontWeight:700, letterSpacing:"0.04em", color, background:bg, border:`1px solid ${border||"transparent"}` }}>
//     {children}
//   </span>
// );
...
// // ─── ATTACK TIMELINE ───────────────────────────────────────────────────────────
// const Timeline = ({ events, pivotalId }) => {
...
//         const dc = LEVEL_COLOR(ev.severity_level || ev.severityLevel || 3);
//         const isOpen = open === ev.id;
//         return (
//           <div key={ev.id} style={{ position:"relative", marginBottom: isPivot?24:16 }}>
...
//                 <div style={{ flex:1 }}>
//                   <div style={{ fontSize:12, fontWeight:600, color:"rgba(255,255,255,0.88)", marginBottom:3 }}>{ev.event_type || ev.eventType}</div>
...
//                   <div style={{ fontSize:9, color:"#3b82f6", marginTop:3 }}>{ev.mitre_tactic || ev.mitreTactic}</div>
//                 </div>
//               </div>
//               {isOpen && ev.raw_log && (
...
//   // Fetch full alert detail (includes timeline) on mount
//   useEffect(() => {
...
//       <div style={{
//         position:"relative", width:"min(880px,96vw)", maxHeight:"90vh",
...
//                 style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:3, padding:"7px 10px", cursor:"pointer", color:"rgba(255,255,255,0.45)", fontSize:14, lineHeight:1 }}>✕</button>
//             </div>
//           </div>
...
//               <Spinner color="#3b82f6"/> Loading full triage data from Claude…
//             </div>
//           ) : (
//             <>
//               <div>
...
//                 <Timeline events={alert.timeline || []} pivotalId={alert.pivotal_event}/>
//               </div>
...
//             </button>
//           )}
//           <button onClick={doFP} style={{ flex:1, padding:"11px 0", borderRadius:3, fontSize:12, fontWeight:600, cursor:"pointer", border:"1px solid rgba(255,255,255,0.09)", background:"rgba(255,255,255,0.03)", color:"rgba(255,255,255,0.45)", transition:"all .15s" }}
...
//         </div>
//         <div style={{ textAlign:"right", flexShrink:0 }}>
//           <div style={{ fontSize:10, color:"rgba(255,255,255,0.28)", fontFamily:"'JetBrains Mono',monospace", marginBottom:6 }}
//             title={alert.created_at ? utcStr(alert.created_at) : ""}>
...
//   if (loading) return <div style={{ color:"rgba(255,255,255,0.3)", fontSize:12, display:"flex", alignItems:"center", gap:8 }}><Spinner color="#3b82f6" size={12}/> Loading metrics…</div>;
//   if (!metrics) return null;

//   const { total=0, true_positive:tp=0, false_positive:fp=0, needs_review:nr=0, hourly_volume=[], top_mitre_tactics=[] } = metrics;
//   const sevDist = [
//     { l:"Critical", v:metrics.critical||0, c:"#ef4444" },
//     { l:"High",     v:metrics.high||0,     c:"#f97316" },
//     { l:"Medium",   v:0,                   c:"#eab308" },
//     { l:"Low",      v:0,                   c:"#3b82f6" },
//   ];
...
//             <div style={{ width:52, fontSize:10, color:"rgba(255,255,255,0.42)" }}>{l}</div>
//             <div style={{ flex:1, height:5, background:"rgba(255,255,255,0.06)", borderRadius:2 }}>
//               <div style={{ width:`${total>0?(v/total)*100:0}%`, height:"100%", background:c, borderRadius:2, opacity:.75 }}/>
//             </div>
//             <div style={{ width:16, fontSize:11, fontWeight:700, color:c, fontFamily:"'JetBrains Mono',monospace", textAlign:"right" }}>{v}</div>
//           </div>
//         ))}
//       </div>

//       {top_mitre_tactics.length > 0 && (
//         <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:3, padding:"12px 14px" }}>
...
//         <div style={{ fontSize:9, color:"rgba(255,255,255,0.28)", marginTop:3 }}>across all triaged alerts</div>
//       </div>
//     </div>
//   );
// };
...
//   // Fetch alerts (polls every 10s for real-time updates)
...
//   // When modal alert changes status, refresh it in the list
...
//               <span style={{ fontSize:10, color:"rgba(255,255,255,.28)", fontFamily:"'JetBrains Mono',monospace" }}>
//                 {backendOk === false ? "BACKEND OFFLINE" : "LIVE · AUTO-REFRESH 10s"}
//               </span>
//             </div>
//           </div>
...
//                 </div>
//               ) : alertsLoading ? (
//                 <div style={{ display:"flex", alignItems:"center", gap:8, color:"rgba(255,255,255,0.3)", fontSize:12, padding:"20px 0" }}>
//                   <Spinner color="#3b82f6" size={14}/> Connecting to backend…
//                 </div>
//               ) : filtered.length === 0 ? (
//                 <div style={{ textAlign:"center", padding:"40px 0", color:"rgba(255,255,255,.2)", fontSize:12, lineHeight:2 }}>
//                   No alerts yet.<br/>
//                   <span style={{ fontSize:10, fontFamily:"'JetBrains Mono',monospace", color:"rgba(255,255,255,.15)" }}>Start the log generator to see alerts appear here.</span>
//                 </div>
//               ) : (
        ...
        <div style={{ width:196, background:"#07070c", borderRight:"1px solid rgba(255,255,255,0.055)", display:"flex", flexDirection:"column", flexShrink:0 }}>
          ...
          <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(255,255,255,0.055)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5 }}>
              <div style={{ width:26, height:26, borderRadius:"50%", background:"linear-gradient(135deg,rgba(59,130,246,.3),rgba(30,64,175,.3))", border:"1px solid rgba(59,130,246,.22)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"#3b82f6" }}>SR</div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:11, fontWeight:600, color:"rgba(255,255,255,.82)" }}>S. Reddy</div>
                <div style={{ fontSize:9, color:"rgba(255,255,255,.3)" }}>L1 Analyst</div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:3 }}>
                <div style={{ width:5, height:5, borderRadius:"50%", background:"#22c55e", animation:"pulse 2s infinite" }}/>
